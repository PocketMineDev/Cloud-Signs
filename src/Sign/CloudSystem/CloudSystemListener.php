    ...
    public function onPacketReceive(DataPacketReceiveEvent $ev) {
        ...
        if ($pk instanceof ModalFormResponsePacket) {
            ...
            if ($id == 100) {
                if ($data !== NULL) {
                    if ($data == 0) {
                        $player->sendMessage($this->plugin->prefix . '§c Error');
                    } elseif ($data == 1) {
                ...
                if ($data !== NULL) {
                    ...
                    if ($query->isOnline()) {
                        ...
                        $player->sendMessage($this->plugin->prefix . '§4 Der Server ist Offline§f!');
                        $this->plugin->player = "";
                    }
                }
            }
        ...
        if ($player->getName() === $this->plugin->player) {
            ...
            if ($tile instanceof Sign) {
                ...
                if ($query->isOnline()) {
                    ...
                    $player->sendMessage($this->plugin->prefix . "§b Erfolgreich§f!");

                } else {
                    $player->sendMessage($this->plugin->prefix . '§c Server ist offline§f!');
                    $this->plugin->player = "";
                }
            }
        } else {
            ...
            if ($tile instanceof Sign) {
                ...
                if($tile->getLine(1) === '§7-= §aBetreten§7 =-'){
                	$player->sendMessage($this->plugin->prefix.'§a Du wirst nun transferiert§f!');
                    $player->transfer($ip, $port);
               } 
              if($tile->getLine(1) === '§7-= §cVoll§7 =-'){
               	$player->sendMessage($this->plugin->prefix.'§c Der Server ist voll§f!');
               } 
              if($tile->getLine(2) === '§eLade Server'){
               	$player->sendMessage($this->plugin->prefix.'§c Der Server ist momentan offline!');
                }
            }
        }
    }
